...
⚠ **ANY change to the size of the firmware turns the `firmware` job red on a
machine without XC8, and it is not obvious why.** `README.md`'s generated
build-size block is filled from XC8's own memory summary, so adding or removing
code makes it stale — and `checkdocs.py --write` cannot refresh it here,
because the summary comes from a build this machine cannot do. Everything
locally stays green and the failure appears only after the push.

**The fix needs no compiler.** The failing job's log carries the summary XC8
just printed. Copy it verbatim into `mplab/build/memory.txt` — the directory is
gitignored, so it is scratch — then run `python tools/checkdocs.py --write` and
commit the README. The figures are still generated by the tool from the
compiler's own output rather than typed into prose, which is the whole point of
that block. **Do not edit the table by hand**; a transcription slip there is
exactly the kind of wrong number the block exists to prevent.
