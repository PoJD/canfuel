# Engine health — the full log, 10–25 September 2026

⚠ **FROZEN ARCHIVE, not the working document.** This is the investigation as
it was written, measurement by measurement, up to 25 September 2026. It is
kept because the tables, fixtures and tools cite it (`tools/idledips.py`,
`tools/coastscan.py`, `frames.md`, `can-decoding.md`), and because the
reasoning behind every entry is here in full. **It is not updated any more.**
Where it and the two newer files disagree, the newer files win:

- **`engine-health.md`** — the working document: current symptoms, the
  hypotheses still open, and the test that decides each one
- **`engine-health-refuted.md`** — every hypothesis settled against, with
  what settled it, and the questions that were answered

Anything below that reads as open may since have been closed there.

---

**What follows is the original text, unchanged.** This is a holding document
for an open investigation, not a firmware document. It exists because one of its questions is genuinely ours — whether
